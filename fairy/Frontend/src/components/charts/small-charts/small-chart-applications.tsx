...
import { usePortfolioCounts } from "@/hooks/use-portfolio-counts"
...
import type { ChartPortfolioData } from "@/types/portfolio"
...
export function SmallChartApplications({ className }: { className?: string }) {
  const { data: portfolioCounts, isLoading, error } = usePortfolioCounts()
  const [activePortfolio, setActivePortfolio] = React.useState<string>("all")
  const [open, setOpen] = React.useState(false)

  // Transform API data to chart format
  const chartData = React.useMemo((): ChartPortfolioData[] => {
    return portfolioCounts.map((item, index) => ({
      portfolio: item.portfolio_name.toLowerCase().replace(/\s+/g, ""),
      applications: item.application_count,
      fill: `var(--color-${item.portfolio_name.toLowerCase().replace(/\s+/g, "")})`,
    }))
  }, [portfolioCounts])
  ...
  const chartConfig = React.useMemo((): ChartConfig => {
    ...
    portfolioCounts.forEach((item, index) => {
      const key = item.portfolio_name.toLowerCase().replace(/\s+/g, "")
      config[key] = {
  ...
  }, [activePortfolio, chartData])

  const portfolios = React.useMemo(() => ["all", ...chartData.map((item) => item.portfolio)], [chartData])
  ...
  }, [activePortfolio, totalApplicants, chartData])
  ...
  // Reset active portfolio when data changes
  React.useEffect(() => {
    setActivePortfolio("all")
  }, [portfolioCounts])
  ...
  if (error) {
    return (
      <Card className={cn("flex flex-col gap-0 pb-0", className)}>
        ...
        <CardContent>
          <div className="flex items-center justify-center h-[250px] text-muted-foreground">Failed to load data</div>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className={cn("flex flex-col gap-0 pb-0", className)}>
      <CardHeader className="flex items-start space-y-0 truncate">
        ...
        {isLoading ? (
          <Skeleton className="ml-auto h-9 w-[150px]" />
        ) : (
                  ...
                  {activePortfolio === "all" ? (
                    ...
                    <>
                      <span
                        ...
                        style={{
                          backgroundColor:
                            chartConfig[activePortfolio as keyof typeof chartConfig]?.color || "transparent",
                        }}
                      />
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    ...
                    >
                      ...
                      <Check
                        className={cn("ml-auto h-4 w-4", activePortfolio === "all" ? "opacity-100" : "opacity-0")}
                      />
                    </CommandItem>
                    {portfolios
                      .filter((p) => p !== "all")
